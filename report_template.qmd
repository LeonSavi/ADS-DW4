...
Each document is represented as a high-dimensional TF-IDF vector (term frequency–inverse document frequency), which captures how important each word is to that document relative to the corpus. K-Means starts by selecting k random “centroids” (one per cluster).
...
### cluster_charts
```{python}
cluster_charts(ml_dct=dct_models,models=test_models,umap_metric ='manhattan')
```
