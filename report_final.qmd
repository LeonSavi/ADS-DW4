...
Another choice we made was to do the lemmatization before removing the stopwords, partly because some words mighr become stopwords only after lemmatization and partly because we wanted to keep the context of the words as much as possible before removing them, since lemmatization is context dependent.
...
Idea: Group similar documents together based on their word patterns.
...
- Assumes clusters are are cicrular if you would represent them in a graph and they are the same size.
- Not probabilistic, each document belongs to exactly one cluster.


### Gaussian Mixture Models (GMM)

Idea: Instead of assigning documents to one cluster, with GMM Each document partly belongs to several clusters.
...
Idea: Finds topics in a collection of documents, like horror or comedy.
...
Idea: Like LDA, it finds topics, but it uses more math instead of probabilities.
...
The factorisation is constrained so that all entries are non-negative, which encourages parts-based, interpretable representations.

Training is usually done by minimizing reconstruction error via iterative updates. 
...
def comparison_charts(df:pd.DataFrame,
    ...
    plt.savefig(f"charts/indicators_svd{'_l10' if labels10 else ''}.png")
    plt.show()
...
Here above we can see the results of the various models on some measurement scales. We graphed the most important ones. We decided to train the models from k2 all the way to k10 so we can see a gradual overview of what happens when increasing the k. We decided to not only include k5 and k10 but also k2. We did this because the dataset includes sentiment labels of 0 and 1 and we wanted to see if the models would cluster the reviews in the same groups as the sentiments. While the models dont explicitly know the sentiment scores, we think that certain words and writing may be associated with positive and negative reviews which the model could pick up on.
...
- SVD vs PCA2: https://www.youtube.com/watch?v=5HNr_j6LmPc
